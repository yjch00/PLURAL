...
class UnifyTask(OFATask):
    def __init__(self, cfg: UnifyConfig, src_dict, tgt_dict):
        ...
        if self.cfg.neg_sample_dir is not None:
            ...
            self.type2ans_dict = None
            self.ans2type_dict = None
            self.all_object_list = None
            self.all_caption_list = None
    ...
    def valid_step(self, sample, model, criterion):
        ...
        logging_output["bert_score"] = scores.sum()

        return loss, sample_size, logging_output


    def _calculate_bleu_scores(self, gen_res, gt_res):
        '''
        ...
        for i in range(len(gts)):
            references.append([gts[i]['caption']])
            predictions.append(res_[i]['caption'])
    ...
    def _calculate_bert_scores(self, gen_res, gt_res):
        ...
        for i in range(len(gts)):
            references.append([gts[i]['caption']])
            predictions.append(res_[i]['caption'])
        scores= bertscore.compute(predictions=predictions, references=references, model_type="bert-base-uncased")
        return scores

    def _calculate_distilbert_scores_(self, gen_res, gt_res):
        '''
        gen_res: generated captions, list of str
        gt_idx: list of int, of the same length as gen_res
        gt_res: ground truth captions, list of list of str.
        ...
        for i in range(len(gts)):
            references.append([gts[i]['caption']])
            predictions.append(res_[i]['caption'])
        scores = bertscore.compute(predictions=predictions, references=references, model_type="distilbert-base-uncased")
        return scores